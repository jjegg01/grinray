...
use cgmath::Vector3;
use grinray::{
    graphics::{Camera, PerspectiveCamera, PerspectiveCameraParams, RayGraphicsContext}, objects::{Cuboid, ObjectTransform, Plane}, CheckerboardMaterial, DebuggerTracer, LinearGRINFresnelMaterial, Scene, Tracer
};
...
fn main() {
    ...
    // Create camera
    let camera = PerspectiveCamera::new(PerspectiveCameraParams {
        pixels: (1024,1024),
        samples: 32,
        ..Default::default()
    });
