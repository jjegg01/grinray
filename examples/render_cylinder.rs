...
use cgmath::Vector3;
use grinray::{
    graphics::{Camera, PerspectiveCamera, PerspectiveCameraParameters, RayGraphicsContext}, objects::{Cylinder, ObjectTransform, Plane}, CheckerboardMaterial, DebuggerTracer, LinearGRINFresnelMaterial, Scene, Tracer, World
};
...
fn main() {
    ...
    let particle = Cylinder::new(1.0, 2.0);
    let particle_transform = ObjectTransform::with_translation((0.0, 0.1, -3.).into());
    // Setup materials
    let plane_mat = CheckerboardMaterial::new((1.0, 1.0, 1.0).into(), Vector3::unit_x());
    let object_mat = LinearGRINFresnelMaterial::new(1.4, (0.0,0.1,0.0).into());
    // Setup scene
    ...
    let particle_mat = scene.add_material(Box::new(object_mat));
